...
$config['AllowedAddresses'] = "75.125.82.64/26,50.97.162.64/26,127.0.0.1";
...
// USED BY DRUPAL CLICKTALE MODULE
if (function_exists('variable_get') && function_exists('drupal_realpath') && function_exists('clicktale_get_path') && function_exists('clicktale_get_logs_path') && function_exists('clicktale_get_cache_path')) {
  $drupal_config = variable_get('clicktale_advanced', array());
  if (!empty($drupal_config)) {
    $config['ScriptsFile'] = drupal_realpath(clicktale_get_path()) . '/ClickTaleScripts.xml';
    $config['CacheFetchingUrl'] = 'http://%ClickTaleCacheUrl%/ClickTaleCache.php?t=%CacheToken%';
    $config['LogPathMask'] = drupal_realpath(clicktale_get_logs_path()) . '/Log_{0}.txt';
    $config['CacheLocation'] = drupal_realpath(clicktale_get_cache_path());
    $config['CacheProvider'] = $drupal_config['method'];
    $config['MaxFolderSize'] = $drupal_config['size'];
    $config['DeleteAfterPull'] = ($drupal_config['delete'] == 'yes' ? TRUE : FALSE);
    $config['MaxCachedSeconds'] = $drupal_config['ttl'];
  }
}
